...
graph TD
    ...
    Traefik -->|smartfound.asykur.dev (Frontend Domain)| Frontend[Frontend Container: Vue 3 + Nginx]
    Traefik -->|smartfoundapi.asykur.dev (Backend Domain)| Backend[Backend Container: PHP 8.2 + Apache]
...
| `APP_URL` | `https://smartfoundapi.asykur.dev` | Public URL of this API |
| `FRONTEND_URL` | `https://smartfound.asykur.dev` | Allowed origin for CORS (Must match frontend URL!) |
...
| `GOOGLE_REDIRECT_URI` | `https://smartfoundapi.asykur.dev/api/v1/auth/google/callback` | Google OAuth redirect endpoint |
...
2. Add your backend API subdomain (e.g., `smartfoundapi.asykur.dev`).
...
1. Go to the **Domains** tab.
2. Add your main frontend domain (e.g., `smartfound.asykur.dev`).
3. Set the **Container Port** to `80`.
4. Enable **HTTPS (Let's Encrypt)**.
...
5. In the Dokploy interface, configure domain routing:
   - Route `smartfound.asykur.dev` to service `frontend` on port `80`.
   - Route `smartfoundapi.asykur.dev` to service `backend` on port `80` (mapped to `80` inside the container).
...
1. **Google Cloud Console Update**:
   ...
   - Add `https://smartfound.asykur.dev` to **Authorized JavaScript Origins**.
   - Add `https://smartfoundapi.asykur.dev/api/v1/auth/google/callback` to **Authorized redirect URIs**.

2. **CORS Validation**:
   - Ensure the backend environment variable `FRONTEND_URL` exactly matches the domain of the frontend (e.g., `https://smartfound.asykur.dev`).
...
- **Error: `Network Error` or CORS issues on login**:
  - Confirm the API base URL is HTTPS: `https://smartfoundapi.asykur.dev/api/v1`.
